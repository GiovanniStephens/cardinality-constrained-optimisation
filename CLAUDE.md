...
python -m src.db purge-phantom-rows --dry-run   # phantom weekend/holiday rows (gotcha 4)
...
4. **Phantom non-NYSE dates (July 2026 incident — root-caused, fixed, purged).** For years, `promote_staging` requested no forward-fill via `ffill_limit=None`, but `load_prices` handed that straight to pandas, where `ffill(limit=None)` means UNLIMITED fill — every promotion stamped each US ticker's previous close onto its 200-ticker chunk-mates' trading days (Tel Aviv Sundays, Asian/European sessions on NYSE holidays). Result: **~921k junk rows** (169,994 weekend + 751,107 on 114 NYSE-closure dates, all verified previous-close duplicates — even SPY carried them on 86 holidays). On the polluted union index SPY read 88% coverage and silently fell out of the backtest frame (found by the beta-1 experiment's Stage-0 run). Now: `load_prices` treats falsy `ffill_limit` as no-fill (root cause); `save_prices` drops weekend-dated rows for US-listed dot-less symbols (write guard); `validate_universe` reports phantom counts on every run (detection — fires automatically post-promotion); `python -m src.db purge-phantom-rows` deletes them with a backup (purged 2026-07-10, backup `portfolio.db.backup_20260710_195650`). The morals stand: **never measure coverage against the raw union date index** (the backtest's `clean_us_price_frame` remains as defence-in-depth), and never pass `None` to a pandas `limit=` expecting "off". One residual to know: foreign dot-suffix listings legitimately keep non-NYSE dates alive in any mixed-listing union index, so a read-time ffill over that index re-materialises phantom values **in memory** even from a clean DB — load with `ffill_limit=None` and fill only after calendar cleaning (`runner.load_backtest_frame` does; the rebalance's 2y loader still fills pre-filter, a small ~10%-of-days vol-damping bias on its estimates, acceptable until its next touch).
...
**After any directory ingest, run `python -m src.db backfill-names`** — the ingest writes
symbols + prices only, and FinanceDatabase has no metadata for post-2021 launches, so
`tickers.name` stays NULL. The rebalance category caps classify **by name**
(`src/categorise.py`); unnamed funds all land in the capped `Unknown` bucket, which made
the caps mathematically infeasible against GA selections (July 2026 incident: ≥4 unnamed
holdings × 5% weight floor > 10% Unknown cap → SLSQP could never converge, silently
falling back to equal weights). Names come from the same directory files' Security Name
column. Then `python -m src.db backfill-volume` for ADV coverage.
