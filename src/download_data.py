...
_VALIDATE_ATTRS = {
    'validate_tickers', '_retry_with_splitting',
}
_CLI_ATTRS = {'main', '_add_file_logging', '_log_final_summary'}
...
def __getattr__(name: str) -> Any:
    ...
    if name in _VALIDATE_ATTRS:
        from src import download_validate
        return getattr(download_validate, name)
    if name in _CLI_ATTRS:
        from src import download_cli
        return getattr(download_cli, name)
    raise AttributeError(f"module {__name__!r} has no attribute {name!r}")


if __name__ == '__main__':
    from src.download_cli import main
    main()
